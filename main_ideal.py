...
if __name__ == '__main__':
    ...
    # Evaluate model
    with torch.no_grad():
        masks, idealMasks = test(model, device, testLoader)

    # 6. MASK SPEECH AND RECONSTRUCT TIME-DOMAIN AUDIO
    # Mask speech
    maskedSpeech = mask_speech(clean_test, noisy_test_spectrogram, clean_test, masks)
    ...
    # for i in range(0, 15):
        ...
        if i < 10:
            ...
            # Second subplot: Mask
            plt.subplot(212)
            plt.imshow(masks[i], origin='lower', aspect='auto')
            plt.colorbar()
            plt.ylim(0, 161)  # Set y-axis limit to 0-161
            plt.xlabel('Time (frames)')
            plt.ylabel('Frequency Bins')
